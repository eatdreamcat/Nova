...
void Engine::PopulateCommandList()
{
    ...
    // Set necessary state. record to bundle while app startup
   /* m_commandList->SetGraphicsRootSignature(m_rootSignature.Get());
...
void Engine::LoadAssets()
{
    // Create an root signature.
    {
        ...
        CD3DX12_DESCRIPTOR_RANGE1 ranges[2]{};
           //   UINT numDescriptors, baseShaderRegister, registerSpace = 0,
        ranges[0].Init(D3D12_DESCRIPTOR_RANGE_TYPE_CBV, 1, 0, 0, D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC);
        ranges[1].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 2, 0, D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC);
    ...
    // Create Constant buffer
    {
        ...
        auto cbHeapProperty = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
        auto cbHeapDesc = CD3DX12_RESOURCE_DESC::Buffer(constantBufferSize);
        ThrowIfFailed(m_device->CreateCommittedResource(
            &cbHeapProperty,
            D3D12_HEAP_FLAG_NONE,
            &cbHeapDesc,
   ...
   // prematurely destroyed.
    ...
    // Create the texture.
    {
        ...
        auto resourcesBarrier = CD3DX12_RESOURCE_BARRIER::Transition(m_texture.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
        m_commandList->ResourceBarrier(1, &resourcesBarrier);
...
std::vector<UINT8> Engine::GenerateTextureData()
{
    ...
    const UINT cellHeight = TextureWidth >> 4;    // The height of a cell in the checkerboard texture.
